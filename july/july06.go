	...
	render := func(surface *blgo.Surface, percent float64) {
		...
		for x := 0; x < int(count-1); x += 1 {
			for y := 0; y < int(count-1); y += 1 {
				...
				surface.MoveTo(p0.x, p0.y+math.Sin(dist(p0.x, p0.y, width/2, height/2)*0.1+amt)*20)
				surface.LineTo(p1.x, p1.y+math.Sin(dist(p1.x, p1.y, width/2, height/2)*0.1+amt)*20)
				surface.LineTo(p2.x, p2.y+math.Sin(dist(p2.x, p2.y, width/2, height/2)*0.1+amt)*20)
				surface.LineTo(p3.x, p3.y+math.Sin(dist(p3.x, p3.y, width/2, height/2)*0.1+amt)*20)
				surface.LineTo(p0.x, p0.y+math.Sin(dist(p0.x, p0.y, width/2, height/2)*0.1+amt)*20)
				s := math.Sin(dist(p0.x, p0.y, width/2, height/2)*0.1 + amt)
				g := color.Grey(blmath.Map(s, -1.0, 1.0, 1.0, 0.25))
